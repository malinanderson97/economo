...
await guarded('7. Housekeeping (items 30, 33–35, M5b)', async () => {
  const H = await page.evaluate(() => {
    ...
    const out = { comments: ['// r = i − πᵉ, with πᵉ = 0 before the PC block', '// Taylor rule, §5', '// λ = (1 − θ)·cred', '// toggle off ⇒ cred frozen', '// eq. 19.5', '// ε = E·P/P*', '// adaptive Eᵉ'].map(has),
      stale: ['§6.3', '§6.7', '§7.8', '§5.5', "Correspondence 6.6'", 'see 5.4', "'i_N'", 'Price flex', '(target), i', '0.4 * sp'].map(has),
  ...
  check('EQ_REF → §6 / §9 / §9 / §8', H.eqRefs.join('|') === 'Model Textbook Correspondence §6|Correspondence §9 *|Correspondence §9 *|Correspondence §8 * (adaptive)', H.eqRefs.join(' | '));
  ...
  check('footnotes → §4.1/§4.2 and §9/§5.2', /§4\.1 \(the factor\) and §4\.2/.test(H.foot) && /§9 price\/expectations dynamics; §5\.2/.test(H.foot), H.foot);
...
await guarded('8. UI smoke (no page errors)', async () => {
