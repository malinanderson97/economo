...
> equations with the trade terms gated off (m=0, no exchange-rate channel) — numerically
> identical to the retired v16. v16 is then archived.
...
`x₁·Y* − n₁·(ε−ε_base)` channel). It is **numerically identical to the retired v16**: same
closed multiplier (k=2.5), same baseline (Y=100). The only difference from old v16 is
cosmetic — the closed result is now printed by the unified (open-form) equations rather than
v16's separate base-deviation formulas; the economics and the numbers are unchanged. Record
one line to this effect in `Model_Textbook_Correspondence` and get Frank's confirmation that
computing the closed economy by gating off the open terms (rather than keeping separate closed
formulas) is fine. This is a yes/no, not a model change.
...
4. **Closed baseline.** UIP locked, default state → Y = 100 ± 0.1 (the gated-off closed
   economy reconciles to the v16 baseline it replaces).
...
7. **Curve reconciles to engine (closed).** UIP locked: the equilibrium point sits ON the
   ...
   the closed IS slope is steeper than the open one (compare `isRateForOutput` at two Y values
   with UIP locked vs unlocked; the closed Δi/ΔY is larger in magnitude because k is larger).
   This is the assertion that catches engine-correct/diagram-wrong: it proves the SAME gated
   function feeds both `solve()` and the plot.
...
- [ ] **Browser check** (verifier-green ≠ diagram-correct): in closed mode the UIP curve and
      its drill-down are hidden/locked, the IS curve redraws with the steeper closed slope,
...
- [ ] `Model_Textbook_Correspondence` updated with one line per the §3 fidelity note (closed
      = open equations with trade terms gated off, numerically identical to v16) — recorded
      BEFORE code per template §3.
