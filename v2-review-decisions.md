...
adjust": at θ = 0, πᵉ is 2% at period 0 and 2% at period 20. At θ = 1 the run diverges
(Blanchard's fixed-nominal-rate instability). All weight figures to be re-run after M2/M4.
...
NOTE: at θ = 0, πᵉ never moves. At θ = 1 the run diverges (Y ≈ 180 by t = 20).
...
### Chat 2 — Document
*Attach*: this file, the md drafts, the build script, the figures table from chat 1.
...
> produced, and that behaviour no longer exists; write it fresh from the new engine's actual
> paths. Don't propose stylistic rewording of anything not on the list.
